...
module.exports = class MenuItem  extends EventEmitter {
  ...
  hasMaxValue(){
    return this.getStateObject().hasMaxValue();
  }
  getMaxValue(){
    return this.getStateObject().getMaxValue();
  }
  hasMinValue(){
    return this.getStateObject().hasMinValue();
  }
  getMinValue(){
  ...
  isCommonTypeAString(){
    return this.getStateObject().isCommonTypeAString();
  }
    ...
    if(this.hasItems())
      ...
      if(this._objectID)
      {
        if(this.isObjectWriteable())
        {
         ...
         else if(this.isCommonTypeANumber() || this.isCommonTypeAString())
         {
  ...
  GetNodeElementsForTelegraph(){
    if(this.hasItems()){
      ...
      const returnString = `${icon} ${this.Name} : ${valueAsString} `;
